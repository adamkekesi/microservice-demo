...
// Package integration holds full-stack tests for the Auth service: real Gin
// router, real GORM against a disposable Postgres (technical plan §8.3).
//
// By default a throwaway Postgres is started via Testcontainers (needs Docker).
// If TEST_DATABASE_URL is set, that database is used instead — handy in
...
func TestMain(m *testing.M) {
	...
	var err error
	if testDB, err = database.Connect("auth-test", dsn); err != nil {
...
// truncate clears the named tables so tests are order-independent.
